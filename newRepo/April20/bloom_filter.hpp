...
public:
  ...
  virtual bool compute_optimal_parameters(unsigned long long int tableSize, unsigned int numOfHashes)
   {
      /*
        Note:
        The following will hard code the table size and the number of 
        hash functions
      */
      ...
      optp.number_of_hashes = numOfHashes;

      std::cout<<std::endl<<" INFO :: NUMBER OF HASHES CHOSEN : " <<optp.number_of_hashes <<std::endl;

...
#define SUCCESS 0
#define FAILURE -1
#define NUM_OF_BFS 20
#define TOTAL_NUM_OF_ELEMENTS 10000
#define INITIAL_NUM_OF_BFS 3
#define REFRESH_TIME 20
#define DEFAULT_BATCH_OPS 100
#define SLEEP_TIME 4 
#define FP_CHECK_COUNTER 2000 
#define DEFAULT_FPR 0.1
#define NUM_INSERTS_CHK_FP 1000
...
public: 
  ...
  int checkFPR() {
    ...
    std::cout<<" INFO:: i value: " <<i <<std::endl;
    ...
    FPR = (double)FP / FP_CHECK_COUNTER;
  ...
  int adjust() {
    int done = FAILURE;
    if ( (numberOfBFs * 2) < NUM_OF_BFS ) {
      numberOfBFs *= 2;
      std::cout<<" INFO:: Increasing BFs in FBF to: " <<numberOfBFs <<std::endl;
      done = SUCCESS;
    }
    if ( (refreshTime / 2) > MINIMUM_REFRESH_TIME ) {
      refreshTime /= 2;
  ...
  for (unsigned long long i = 0; i < TOTAL_NUM_OF_ELEMENTS; i++ ) {
    ...
    if ( 0 == i % NUM_INSERTS_CHK_FP ) {
      ...
      if ( FAILURE == rc ) { 
        rc = oFBF.adjust();
        if ( FAILURE == rc ) {
	  std::cout<<" INFO:: Unable to adjust FBF size or refresh time. Break " <<std::endl;
          break;
        }
      } 
    }
